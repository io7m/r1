...
@SuppressWarnings("synthetic-access") @EqualityReference public final class KDistanceRenderer implements
  KDistanceRendererType
{
  ...
  public static KDistanceRendererType newRenderer(
    final JCGLImplementationType g,
    final KShaderCacheDepthType shader_cache,
    final LogUsableType log)
    throws RException
  {
    return new KDistanceRenderer(g, shader_cache, log);
  }
        ...
        new KMatricesInstanceFunctionType<Unit, JCGLException>() {
          @Override public Unit run(
            ...
            throws JCGLException,
              RException
          {
            KDistanceRenderer.renderDepthPassInstance(gc, mwi, jp, i, faces);
            return Unit.unit();
          }
        });
    }
      ...
      RException
  {
    ...
    try {
      ...
      KShadingProgramCommon.putDistanceRangeReuse(jp);
    ...
    final LogUsableType in_log)
  {
    ...
    this.code_map = KDistanceRenderer.makeCodeMap();
    ...
    for (final String depth_code : instances.getMaterialCodes()) {
      ...
      exec.execRun(new JCBExecutorProcedureType<RException>() {
            ...
            RException
        {
          ...
          KShadingProgramCommon.putDistanceRange(
            jp,
            projection.projectionGetZFar());

          KDistanceRenderer.renderDepthPassBatch(gc, mwo, jp, batch, faces);
        }
      });
    }
  }

  @Override public void rendererEvaluateDistance(
    final PMatrixI4x4F<RSpaceWorldType, RSpaceEyeType> view,
    final KProjectionType projection,
    final KDepthInstancesType instances,
    final KFramebufferDistanceUsableType framebuffer,
    final OptionType<KFaceSelection> faces)
    throws RException
  {
    ...
    final FramebufferUsableType fb = framebuffer.getDistancePassFramebuffer();

    try {
      gc.framebufferDrawBind(fb);
      try {
        this.rendererEvaluateDistanceWithBoundFramebuffer(
          view,
          projection,
          instances,
          framebuffer.getArea(),
  ...
  {
    return KDistanceRenderer.NAME;
  }

  @Override public void rendererEvaluateDistanceWithBoundFramebuffer(
    final PMatrixI4x4F<RSpaceWorldType, RSpaceEyeType> view,
    final KProjectionType projection,
    final KDepthInstancesType instances,
    ...
    try {
      this.matrices.withObserver(
        ...
        new KMatricesObserverFunctionType<Unit, JCGLException>() {
          ...
          {
            try {
              KDistanceRenderer.this.renderScene(
                gc,
                projection,
                instances,
                framebuffer_area,
      ...
      JCacheException
  {
    ...
    gc.scissorDisable();
    gc.viewportSet(area);
    this.renderDepthPassBatches(projection, instances, gc, mwo, faces);
  }
}
