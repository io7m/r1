...
public final class ForwardMakeAll
{
      ...
      ConstraintError
  {
    ...
    final List<Label> labels = Labels.allLabels();
    ForwardMakeAll.makeSources(labels, out_parasol_dir);
    ForwardMakeAll.makeBatch(labels, out_batch);
  ...
  public static void makeBatch(
    final @Nonnull List<Label> labels,
    ...
    throws IOException
  {
    ...
    for (final Label l : labels) {
      ...
      final FileWriter writer = new FileWriter(file);
      try {
        writer.append(Shaders.moduleForward(l));
      } finally {
        writer.flush();
        writer.close();
      }
