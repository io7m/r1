...
module Shadow is
  ...
  type shadow_basic is record
    depth_bias : float,
    factor_min : float,
    factor_max : float
  end;

  function basic_factor (
    config        : shadow_basic,
    ...
    if F.lesser (p [w], 0.0) then
      ...
      let
        value current_tex =
          T.clip_to_texture (p);
        value map_depth =
          S2.texture (t_scene_depth, current_tex [x y]) [x];
        value map_depth_adjusted =
          F.add (map_depth, config.depth_bias);
      in
        if F.lesser (map_depth_adjusted, current_tex [z]) then
          config.factor_max
        else
          config.factor_min
        end
  ...
  function basic_factor_packed4444 (
    config        : shadow_basic,
    t_scene_depth : sampler_2d,
    p             : vector_4f
  ) : float =
    if F.lesser (p [w], 0.0) then
      ...
      let
        value current_tex =
          T.clip_to_texture (p);
        value rgba =
          S2.texture (t_scene_depth, current_tex [x y]);
        value map_depth =
          P.unpack4444 (rgba);
        value map_depth_adjusted =
          F.add (map_depth, config.depth_bias);
  ...
  shader fragment shadow_BC_f is
    ...
    parameter shadow_basic   : shadow_basic;
    out out_0                : vector_4f as 0;
  with
    value f =
      basic_factor (shadow_basic, t_scene_depth, f_position_scene_clip);
    value rgba =
  ...
  with
    value f =
      basic_factor_packed4444 (shadow_basic, t_scene_depth, f_position_scene_clip);
    value rgba =
      new vector_4f (f, f, f, 1.0);
  as
    out out_0 = rgba;
