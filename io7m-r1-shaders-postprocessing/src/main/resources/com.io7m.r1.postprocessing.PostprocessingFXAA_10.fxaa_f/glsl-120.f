...
)
{
    ...
    if(earlyExit)
        ...
        float lumaNW = FxaaLuma(texture2DLod(tex, posM + (ivec2(-1,-1) * fxaaQualityRcpFrame.xy), 0.0));
        float lumaSE = FxaaLuma(texture2DLod(tex, posM + (ivec2( 1, 1) * fxaaQualityRcpFrame.xy), 0.0));
        float lumaNE = FxaaLuma(texture2DLod(tex, posM + (ivec2( 1,-1) * fxaaQualityRcpFrame.xy), 0.0));
        float lumaSW = FxaaLuma(texture2DLod(tex, posM + (ivec2(-1, 1) * fxaaQualityRcpFrame.xy), 0.0));
    ...
    if( horzSpan) posM.y += pixelOffsetSubpix * lengthSign;
        ...
        return vec4(texture2DLod(tex, posM, 0.0).xyz, rgbyM.y);
