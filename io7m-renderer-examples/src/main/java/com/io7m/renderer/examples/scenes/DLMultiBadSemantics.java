...
public final class DLMultiBadSemantics implements ExampleSceneType
{
  /**
   * Construct the example.
   */

  public DLMultiBadSemantics()
  {
    ...
    throws RException
  {
    ...
    final KTransformType mt_left =
      KTransformOST.newTransform(QuaternionI4F.IDENTITY, new VectorI3F(
        1.0f,
        1.0f,
        1.0f), new RVectorI3F<RSpaceWorldType>(-0.5f, 1.0f, 0.0f));

    final KInstanceOpaqueRegular m_left =
    ...
    final KTransformType mt_right =
      KTransformOST.newTransform(QuaternionI4F.IDENTITY, new VectorI3F(
        1.0f,
        1.0f,
        1.0f), new RVectorI3F<RSpaceWorldType>(0.5f, 1.0f, 0.0f));

    final KInstanceOpaqueRegular m_right =
      KInstanceOpaqueRegular.newInstance(
        scene.mesh("sphere32.rmbz"),
        mmb.build(),
        mt_right,
...

    {
      ...
      gb0.groupAddInstance(m_left);
      gb0.groupAddInstance(m_right);

      final KLightDirectionalBuilderType b = KLightDirectional.newBuilder();

      {
        final VectorI3F d_raw =
          VectorI3F.normalize(new VectorI3F(1.0f, -1.0f, -1.0f));
...

    {
      ...
      gb1.groupAddInstance(m_left);
      gb1.groupAddInstance(m_right);
