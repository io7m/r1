...
final class VGLImplementation
{
    ...
    final LogUsableType in_log)
  {
    ...
    if (GLProfile.isAvailable(GLProfile.GL3)) {
      return NullCheck.notNull(GLProfile.get(GLProfile.GL3));
    }
    if (GLProfile.isAvailable(GLProfile.GLES3)) {
      return NullCheck.notNull(GLProfile.get(GLProfile.GLES3));
    }
    if (GLProfile.isAvailable(GLProfile.GL2GL3)) {
      return NullCheck.notNull(GLProfile.get(GLProfile.GL2GL3));
    }

    in_log.debug("No preferred contexts available, falling back to default");
    return NullCheck.notNull(GLProfile.getDefault());
  }
