...
import com.io7m.r1.types.RSphericalTransform;
...
@SuppressWarnings("static-method") public final class RSphericalTransformTest
{
  public void testIdentity()
  {
    ...
    final PVectorM3F<RSpaceEyeType> decoded = new PVectorM3F<RSpaceEyeType>();
    ...
    for (float x = 0.0f; x <= 1.0f; x += 0.01f) {
      for (float y = 0.0f; y <= 1.0f; y += 0.01f) {
        for (float z = 0.0f; z <= 1.0f; z += 0.01f) {
          ...
          final PVectorI3F<RSpaceEyeType> v =
            new PVectorI3F<RSpaceEyeType>(vn.getXF(), vn.getYF(), vn.getZF());
