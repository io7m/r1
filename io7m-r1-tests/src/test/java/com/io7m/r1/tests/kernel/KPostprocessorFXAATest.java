...
@SuppressWarnings("static-method") public final class KPostprocessorFXAATest
{
    ...
    throws Exception
  {
    ...
    for (final Quality q : KFXAAParameters.Quality.values()) {
      assert q != null;

      final KFXAAParametersBuilderType fxb = KFXAAParameters.newBuilder();
      fxb.setQuality(q);
      final KFXAAParameters fxaa_config = fxb.build();
  ...
  @Test public void testParameters()
  {
    QuickCheck.forAllVerbose(
      new KFXAAParametersGenerator(),
      new AbstractCharacteristic<KFXAAParameters>() {
        @Override protected void doSpecify(
          final KFXAAParameters p)
          throws Throwable
        {
          assert p != null;
          final KFXAAParametersBuilderType b =
            KFXAAParameters.newBuilderFrom(p);
          final KFXAAParameters q = b.build();
